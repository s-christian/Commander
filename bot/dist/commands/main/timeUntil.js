...
class SayCommand extends CustomCommand_1.default {
    ...
    async run(message, date) {
        if (!date)
            return message.reply("you must provide a date. Please see `!help timeuntil` for proper usage.");
        const now = dayjs_1.default();
        const future = dayjs_1.default(date, "M/DD/YYYY h:mm:ss A");
        if (!future.isValid())
        ...
        return message.say(`${years} year(s), ${months} month(s), ${days} day(s), ${hours} hour(s), ${minutes} minute(s), ${seconds} second(s).`);
    }
}
